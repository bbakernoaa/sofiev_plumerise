...
def mock_fire_emission_generator(tmp_path):
    ...
    climo_ds = xr.Dataset(
        {
            "emissions": (
                ("month", "lat", "lon"),
                da.ones((12, 10, 10), chunks=(12, 5, 5)),
            )
        },
    ...
    generator = FireEmissionGenerator(
        model_path=str(model_path), climo_path=str(climo_path)
    )
...
def test_run_step_dask_awareness(mock_fire_emission_generator):
    ...
    ufs_met = xr.Dataset(
        {
            "t2m": (
                ("time", "lat", "lon"),
                da.full((1, 10, 10), 295.0, chunks=(1, 5, 5)),
            ),
            "rh2m": (
                ("time", "lat", "lon"),
                da.full((1, 10, 10), 60.0, chunks=(1, 5, 5)),
            ),
            "u10": (
                ("time", "lat", "lon"),
                da.full((1, 10, 10), 5.0, chunks=(1, 5, 5)),
            ),
            "v10": (
                ("time", "lat", "lon"),
                da.full((1, 10, 10), 5.0, chunks=(1, 5, 5)),
    ...
    memory_grid = xr.DataArray(
        da.ones((10, 10), chunks=(5, 5)), coords=[lats, lons], dims=["lat", "lon"]
    )
    ...
    emissions, new_states = mock_fire_emission_generator.run_step(
