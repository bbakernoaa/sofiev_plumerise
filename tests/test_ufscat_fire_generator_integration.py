...
def setup_test_data(tmp_path):
    ...
    met_ds = xr.Dataset({
        't2m': (('time', 'lat', 'lon'), np.full((1, len(lats), len(lons)), 298.0, dtype=np.float32)),
        'rh2m': (('time', 'lat', 'lon'), np.full((1, len(lats), len(lons)), 45.0, dtype=np.float32)),
        'u10': (('time', 'lat', 'lon'), np.full((1, len(lats), len(lons)), 3.0, dtype=np.float32)),
        'v10': (('time', 'lat', 'lon'), np.full((1, len(lats), len(lons)), 3.0, dtype=np.float32)),
        'precip': (('time', 'lat', 'lon'), np.zeros((1, len(lats), len(lons)), dtype=np.float32)),
    }, coords=coords_3d)
    ...
    states_ds = xr.Dataset({
        'ffmc': (('lat', 'lon'), np.full((len(lats), len(lons)), 88.0, dtype=np.float32)),
        'dmc': (('lat', 'lon'), np.full((len(lats), len(lons)), 90.0, dtype=np.float32)),
        'dc': (('lat', 'lon'), np.full((len(lats), len(lons)), 350.0, dtype=np.float32)),
    }, coords=coords_2d)
...
def test_fire_emission_generator_integration(setup_test_data):
    ...
    assert 'history' in final_emissions.attrs, "History attribute missing from emissions"
    ...
    output_states_path = os.path.join(os.path.dirname(paths["model_path"]), "new_fwi_states.nc")
